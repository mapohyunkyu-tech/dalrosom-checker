...
def recommended_writer_perspective(field="", usecase_mode=""):
    ...
    if usecase_mode in ["카페 정보성", "커뮤니티/바이럴", "후기/리뷰"]:
        return "정보성 블로그 작성자"
    if field == "에스테틱 / 피부관리":
        return "에스테틱 원장"
    if field == "병원 / 의료":
...
ARTICLE_STYLES = ["일반 정보성", "대표 작성형 정보성", "매출 전환형 정보성", "홈판 후킹형"]
...
def brand_voice_block(article_style="일반 정보성", sub_keywords="", brand_name="", conversion_goal="", brand_intensity="업체명 없음", tone_detail=""):
    ...
    style_rules = {
        ...
        "홈판 후킹형": "- 글 성격은 홈판 후킹형이다. 제목과 첫문장은 웃기거나 생활감 있게 멈추게 만들되, 본문은 낚시 없이 정보성으로 받쳐준다. 과장·허위·자극만 있는 제목은 금지한다.",
    }
...
- 글 성격 “{article_style}”을 유지하고, 일반 정보성/대표 작성형/매출 전환형/홈판 후킹형을 임의로 바꾸기 금지

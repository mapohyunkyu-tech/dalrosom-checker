...
st.set_page_config(page_title="달로썸 원고 검수기 v6.5", layout="wide")
...
def build_claude_prompt(voice_type, intro_type, title_type, keyword, field, body_text="", first_sentence_type="자동 추천", homepage_mode="홈페이지 정보 없음", homepage_info="", usecase_mode="블로그 정보성", writer_perspective="정보성 블로그 작성자"):
...
st.caption("GPT 조사 프롬프트 → 자료등급/고민패턴/화법 선택/감정흐름/제목유형/도입8가지 → GPTs용 프롬프트 → 초안 검수 → Claude 윤문 지시까지 한 흐름으로 사용합니다. v6.5에서는 작성자 관점을 ①/② 앞으로 빼고, ② 조사 결과 붙여넣기 화면의 홈페이지 입력칸을 제거했습니다.")
...
with tab_research:
    ...
    research_prompt = build_research_prompt(r_topic, r_keyword, r_field, r_goal, r_extra, r_target_len, r_spacing_type, r_paragraph_option, r_intro_type, r_title_type, r_voice_choice, r_first_sentence_type, r_homepage_mode, r_homepage_info, r_homepage_url, r_keyword_delivery_text, r_keyword_placement_text, r_usecase_mode, r_writer_perspective)
...
with tab_design:
    ...
    if d_use_research_inputs:
        ...
        st.info(f"① 입력값 적용: 주제={d_topic} / 키워드={d_keyword} / 분야={d_field} / 사용처={d_usecase_mode} / 작성자 관점={d_writer_perspective} / 분량={d_spacing_type} {d_target_len}자 내외")
        with d_col1:
    ...
    draft_prompt = build_draft_prompt(d_topic, d_keyword, d_field, d_content_type, d_voice, d_intro_type, d_title_type, a_lines, b_lines, c_lines, d_extra_rules, d_target_len, d_spacing_type, d_paragraph_option, d_prompt_mode, d_first_sentence_type, d_homepage_mode, d_homepage_info, d_keyword_delivery_text, d_keyword_placement_text, d_usecase_mode, d_writer_perspective)
    claude_prompt_empty = build_claude_prompt(d_voice, d_intro_type, d_title_type, d_keyword, d_field, first_sentence_type=d_first_sentence_type, homepage_mode=d_homepage_mode, homepage_info=d_homepage_info, usecase_mode=d_usecase_mode, writer_perspective=d_writer_perspective)
    ...
    st.text_area(
        ...
        value=draft_prompt,
        height=520,
        key=dynamic_widget_key("draft_prompt_live", d_topic, d_keyword, d_field, d_usecase_mode, d_writer_perspective, d_voice, d_intro_type, d_first_sentence_type, d_title_type, d_target_len, research_text)
    )
    ...
    st.text_area(
        ...
        height=420,
        key=dynamic_widget_key("claude_design_live", d_voice, d_intro_type, d_title_type, d_keyword, d_field, d_first_sentence_type, d_homepage_mode, d_homepage_info, d_usecase_mode, d_writer_perspective)
    )
...
with tab_check:
    ...
    with st.sidebar:
        ...
        if use_flow_check:
            ...
            st.info(f"자동 적용: 분야={field} / 사용처={selected_usecase_mode} / 작성자 관점={writer_perspective} / 키워드={keyword} / 제목유형={selected_title_type} / 화법={selected_voice_type} / 도입방식={selected_intro_type} / 첫문장={selected_first_sentence_type}")
    ...
    if st.button("검수 시작", type="primary"):
        ...
        st.text_area(
            ...
            height=520,
            key=dynamic_widget_key("claude_check_live", selected_usecase_mode, writer_perspective, selected_voice_type, selected_intro_type, selected_first_sentence_type, selected_title_type, keyword, field, homepage_mode, homepage_info, title, body[:500])
        )

...
def recommended_writer_perspective(field="", usecase_mode=""):
    """분야/사용처에 맞는 기본 작성자 관점. 에스테틱 원장이 기본으로 고정되는 문제를 막는다."""
    field = field or ""
...
def writer_index(field="", usecase_mode="", saved_value=None):
    value = saved_value or recommended_writer_perspective(field, usecase_mode)
...
def brand_voice_block(article_style="일반 정보성", sub_keywords="", brand_name="", conversion_goal="", brand_intensity="업체명 없음", tone_detail=""):
    ...
    tone_detail = (tone_detail or "").strip() or default_tone_by_article_style(article_style)
    brand_rules = []
    ...
    lines = ["[글 성격 / 브랜드 반영 / 말투 세부 조건]", f"- 글 성격: {article_style}", f"- 말투 세부 지시: {tone_detail}", style_rules.get(article_style, style_rules["일반 정보성"])]
...
0-2. 작성자 관점은 “{writer_perspective}”이다. 다른 분야의 원장/전문의/변호사 관점으로 바꿔 쓰지 마. 특히 작성자 관점이 에스테틱 원장이 아닐 때는 에스테틱 원장 말투, 피부관리실 상담 말투, ‘원장으로서’ 표현을 쓰지 마.
...
def build_claude_prompt(voice_type, intro_type, title_type, keyword, field, body_text="", first_sentence_type="자동 추천", homepage_mode="홈페이지 정보 없음", homepage_info="", usecase_mode="블로그 정보성", writer_perspective="정보성 블로그 작성자", article_style="일반 정보성", sub_keywords="", brand_name="", conversion_goal="", brand_intensity="업체명 없음", tone_detail=""):
    ...
    brand_guard = brand_voice_block(article_style, sub_keywords, brand_name, conversion_goal, brand_intensity, tone_detail)
...
with tab_check:
    ...
    with st.sidebar:
        ...
        if use_flow_check:
            ...
            auto_writer_perspective = st.session_state.get("applied_writer_perspective", st.session_state.get("r_writer_perspective", recommended_writer_perspective(field, selected_usecase_mode)))

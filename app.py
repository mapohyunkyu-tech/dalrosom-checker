...
st.set_page_config(page_title="달로썸 원고 검수기 v7.8", layout="wide")
...
FIELDS = ["에스테틱 / 피부관리", "병원 / 의료", "법률", "청소 / 홈케어", "생활용품", "학원 / 교육", "인테리어 / 리모델링", "보험 / 금융 / 부동산", "맛집 / 여행 / 숙박", "주식 / 코인 / 투자", "홈피드 / 생활 이슈", "기타 전문업종"]
WRITER_PERSPECTIVES = [
    "에스테틱 원장",
    "해당 분야 병원 대표원장/전문의",
    "피부과 원장/전문의",
...
def recommended_writer_perspective(field="", usecase_mode="", topic="", keyword=""):
    """분야/사용처/주제에 맞는 기본 작성자 관점. 병원 분야는 주제·키워드로 세부 진료과를 자동 추천한다."""
    field = field or ""
    ...
    if field == "법률":
        return "변호사"
    if field in ["청소 / 홈케어", "학원 / 교육", "인테리어 / 리모델링", "보험 / 금융 / 부동산", "맛집 / 여행 / 숙박", "기타 전문업종", "기타"]:
        return "전문업종 대표"
    return "정보성 블로그 작성자"
...
ARTICLE_STYLES = ["일반 정보성", "대표 작성형 정보성", "매출 전환형 정보성", "대표 작성형 + 매출 전환형 정보성", "홈피드 후킹형", "홈피드 수익형 블로그 글"]
...
def default_tone_by_article_style(article_style="일반 정보성", writer_perspective="정보성 블로그 작성자"):
    style = article_style or "일반 정보성"
    writer_perspective = writer_perspective or "정보성 블로그 작성자"
    if style == "대표 작성형 정보성":
        if is_hospital_writer(writer_perspective):
            return "대표원장/전문의가 진료실에서 환자에게 직접 설명하듯 작성한다. 딱딱한 의학 설명만 나열하지 말고 ‘진료실에서 자주 듣는 질문입니다’, ‘환자분들이 가장 헷갈려하는 부분입니다’, ‘상담 때는 이 기준을 먼저 확인합니다’처럼 실제 설명하는 말맛을 본문 2~3곳에 자연스럽게 넣는다. 다만 치료 효과 보장, 과한 홍보, 가짜 경험담은 쓰지 않는다."
        return "대표가 독자에게 직접 설명하듯 쓰되, 딱딱한 ‘~입니다/합니다’ 반복은 줄이고 사람이 쓴 듯한 말맛을 섞은 정보성 문체"
    if style == "매출 전환형 정보성":
        if is_hospital_writer(writer_perspective):
            return "의료진이 상담 기준을 차분히 설명하듯 쓰고, 확인된 병원 정보가 있을 때만 부드럽게 연결한다. 치료 효과를 보장하지 말고, 환자가 무엇을 먼저 확인해야 하는지 중심으로 쓴다."
        return "정보성 기준을 먼저 설명하고, 확인된 업체 장점과 상담·체험·문의 흐름을 자연스럽게 연결하는 부드러운 전환형 문체"
    if style == "대표 작성형 + 매출 전환형 정보성":
        if is_hospital_writer(writer_perspective):
            return "대표원장/전문의가 진료실에서 환자에게 직접 설명하듯 작성한다. 본문 2~3곳에 ‘진료실에서 자주 묻는 질문입니다’, ‘상담 때는 이 기준을 먼저 확인합니다’ 같은 말맛을 넣되, 정보성 기준을 먼저 설명하고 정밀검사·상담 등 전환 목표로 자연스럽게 연결한다. 치료 효과 보장, 과한 홍보, 가짜 경험담은 쓰지 않는다."
        return "대표가 직접 기준을 설명하듯 쓰되, 정보성 기준을 먼저 제시하고 상담·체험·문의 흐름을 자연스럽게 연결하는 문체"
    if style in ["홈피드 후킹형", "홈피드 수익형 블로그 글", "홈판 후킹형"]:
...
def brand_voice_block(article_style="일반 정보성", sub_keywords="", brand_name="", conversion_goal="", brand_intensity="업체명 없음", tone_detail="", writer_perspective="", field="", topic="", keyword=""):
    article_style = article_style or "일반 정보성"
    ...
    style_rules = {
        "일반 정보성": "- 글 성격은 일반 정보성이다. 제3자가 기준을 정리해주는 느낌으로 쓰고, 업체 홍보는 최소화한다.",
        "대표 작성형 정보성": "- 글 성격은 대표 작성형 정보성이다. 대표가 독자에게 직접 설명하듯 쓰되, 가짜 후기나 과장 홍보가 아니라 운영 기준을 알려주는 느낌으로 쓴다. ‘~입니다/합니다’만 반복하지 말고 ‘~해요’, ‘~볼 수 있어요’, ‘~놓치기 쉽습니다’ 같은 자연스러운 말맛을 섞는다.",
        "매출 전환형 정보성": "- 글 성격은 매출 전환형 정보성이다. 정보성 60%, 공감 20%, 자연스러운 전환 20% 정도로 유지한다. 먼저 선택 기준을 설명하고, 확인된 업체 장점과 전환 목표를 뒤에서 연결한다.",
        "대표 작성형 + 매출 전환형 정보성": "- 글 성격은 대표 작성형 + 매출 전환형 정보성이다. 대표원장/전문의가 직접 설명하듯 기준을 풀어주되, 정보성 60%, 공감 20%, 자연스러운 전환 20% 정도로 유지한다. 병원명이나 장점을 임의로 만들지 말고, 독자가 검사·상담으로 확인해야 할 기준을 자연스럽게 남긴다.",
        "홈피드 후킹형": "- 글 성격은 홈피드 후킹형이다. 제목과 첫문장은 웃기거나 생활감 있게 멈추게 만들되, 본문은 낚시 없이 정보성으로 받쳐준다. 과장·허위·자극만 있는 제목은 금지한다.",
        "홈피드 수익형 블로그 글": "- 글 성격은 홈피드 수익형 블로그 글이다. 검색 키워드보다 생활 이슈성, 후킹 제목, 첫문장 공감, 말맛, 체류시간을 우선한다. 애드포스트 수익·홈피드 노출·구매전환을 보장하지 않는다.",
        "홈판 후킹형": "- 글 성격은 홈판 후킹형이다. 단, 네이버 명칭은 홈피드로 표기한다. 제목과 첫문장은 웃기거나 생활감 있게 멈추게 만들되 본문은 낚시 없이 정보성으로 받쳐준다.",
    }
    lines = ["[글 성격 / 브랜드 반영 / 말투 세부 조건]", f"- 글 성격: {article_style}", f"- 작성자 관점: {writer_perspective if writer_perspective else '정보성 블로그 작성자'}", f"- 말투 세부 지시: {tone_detail}", style_rules.get(article_style, style_rules["일반 정보성"])]
    if field == "병원 / 의료" or is_hospital_writer(writer_perspective):
        ...
        if article_style in ["대표 작성형 정보성", "대표 작성형 + 매출 전환형 정보성"]:
        ...
        if article_style in ["매출 전환형 정보성", "대표 작성형 + 매출 전환형 정보성"] or conversion_goal:
    ...
    lines.append("- 공식 홈페이지/업체 정보가 제공된 경우에는 확인된 장점·철학·상담 기준 중 이번 주제와 직접 연결되는 내용을 마무리 문단에 반드시 1~2문장 반영한다. 단, 자료에 없는 장점은 만들지 않는다.")
    return "\n".join(lines)
...
RESEARCH_FIELDS = [
    ...
    "인테리어 / 리모델링", "보험 / 금융 / 부동산", "맛집 / 여행 / 숙박", "주식 / 코인 / 투자", "홈피드 / 생활 이슈", "기타"
]
...
- 홈페이지 정보가 있을 때만 업체·병원·법무법인 철학이나 장점을 마무리에 1~2문장 반영한다."""
    ...
    elif "금융" in field or "보험" in field or "부동산" in field or "주식" in field or "코인" in field or "투자" in field:
        field_guard = "\n[분야별 추가 주의 - 금융/투자]\n- 수익 보장, 손실 없음, 무조건 유리함, 매수/매도 권유, 특정 종목·코인 추천처럼 보이는 단정 표현을 피한다."
    return base + "\n" + body + field_guard
...
- 공식 홈페이지에서 확인되는 장점·철학·상담 기준이 있으면 이번 주제와 실제로 연결되는 내용만 골라 마무리 반영 후보를 반드시 1~2문장으로 만든다.
...
확인된 내용 중 이번 주제와 연결 가능한 장점·철학·상담 기준은 마무리 반영 후보로 반드시 1~2문장 정리해줘.
...
중요: 공식 홈페이지에서 확인된 경우에는 확인된 병원/업체/법무법인 장점·철학·상담 기준 중 이번 주제와 연결되는 내용을 마무리에 반드시 1~2문장 반영하도록 설계한다.
...
- 공식 홈페이지에서 확인된 정보가 주제와 관련 있으면 마무리 반영은 필수이며 1~2문장 정도로 제한한다.
...
작성 지시:
0. {title_rule}
0-1. 원고 사용처 “{usecase_mode}”에 맞는 말투와 금지조건을 반드시 지킨다. 문단 뼈대는 유지하되, 블로그/카페/커뮤니티/후기형/홈피드형에 맞게 문장 길이와 마무리 방식을 조정한다.
0-2. 작성자 관점은 “{writer_perspective}”이다. 단, 병원/의료 원고에서 지정 관점이 주제와 명백히 맞지 않으면 주제와 키워드에 맞는 진료과 관점을 우선한다. 작성자 관점이 이미 주제와 맞으면 임의로 다른 과목 관점으로 바꾸지 마.
0-3. 위 [글 성격 / 브랜드 반영 / 말투 세부 조건]을 반드시 지킨다. 선택한 글 성격에 맞게 업체명·전환 목표·말맛을 조절한다. “대표 작성형 + 매출 전환형 정보성”은 대표 말투와 전환 흐름을 함께 반영하되 과장 홍보는 피한다.
...
10. 마무리는 강한 구매/상담 유도보다 독자가 자기 상황을 확인하게 만드는 방향으로 작성해줘. 단, 홈페이지/업체 정보가 제공된 경우에는 확인된 장점·철학·상담 기준 중 이번 주제와 직접 연결되는 내용을 마무리에 반드시 1~2문장 포함해줘.
...
- 글 성격 “{article_style}”을 유지하고, 일반 정보성/대표 작성형/매출 전환형/홈피드 후킹형을 임의로 바꾸기 금지
...
with tab_check:
    ...
    with st.sidebar:
        ...
        if use_flow_check:
            ...
            st.info(f"자동 적용: 분야={field} / 사용처={selected_usecase_mode} / 작성자 관점={writer_perspective} / 글 성격={article_style} / 업체명={brand_name or '없음'} / 전환목표={conversion_goal or '없음'} / 키워드={keyword} / 제목유형={selected_title_type} / 화법={selected_voice_type} / 도입방식={selected_intro_type} / 첫문장={selected_first_sentence_type}")

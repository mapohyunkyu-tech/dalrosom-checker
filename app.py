...
st.set_page_config(page_title="달로썸 원고 검수기 v7.6", layout="wide")
...
def brand_voice_block(article_style="일반 정보성", sub_keywords="", brand_name="", conversion_goal="", brand_intensity="업체명 없음", tone_detail="", writer_perspective="", field="", topic="", keyword=""):
    ...
    if sub_keywords:
        brand_rules.append(f"- 보조키워드 ‘{sub_keywords}’는 핵심 키워드처럼 반복하지 말고 필요한 문단에 1~2회씩 자연스럽게 섞는다.")
    if conversion_goal:
    ...
    if field == "병원 / 의료" or is_hospital_writer(writer_perspective):
        ...
        lines.append("- 작성자 관점을 다른 과목으로 바꾸지 않는다. 예: C-ARM 주사치료를 피부과 원장 말투로 쓰지 말고 통증·척추관절 진료를 설명하는 의료진 말투로 쓴다.")
        if article_style == "대표 작성형 정보성":
            ...
            lines.append("- 단, ‘저희 병원은’, ‘풍부한 경험’, ‘정직한 진료’, ‘최신 장비’ 같은 병원 장점·철학은 공식 홈페이지나 사용자가 제공한 자료에 있을 때만 쓴다.")
...
0-2. 작성자 관점은 “{writer_perspective}”이다. 다른 분야의 원장/전문의/변호사 관점으로 바꿔 쓰지 마. 특히 작성자 관점이 에스테틱 원장이 아닐 때는 에스테틱 원장 말투와 피부관리실 상담 말투를 쓰지 마. 병원 대표원장/전문의 관점이면 해당 진료과 원장이 설명하는 말투는 허용하되, 다른 과목 관점으로 바꾸지 마.
0-3. 위 [글 성격 / 브랜드 반영 / 말투 세부 조건]을 반드시 지킨다. 일반 정보성과 대표 작성형 정보성의 톤을 섞지 말고, 선택한 글 성격에 맞게 업체명·전환 목표·말맛을 조절한다.

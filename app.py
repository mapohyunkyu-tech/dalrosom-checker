...
config = secret_config() or saved_config()
...
with tabs[4]:
    ...
    auth_mode = st.selectbox("인증 방식", ["hub", "legacy_ncp", "developer"], index=["hub","legacy_ncp","developer"].index(settings.get("auth_mode","hub")))
    ...
    if c1.button("저장", type="primary", use_container_width=True):
        ...
        save_settings(settings); st.success("저장했습니다. 앱을 새로고침하면 적용됩니다.")
...
with tabs[1]:
    ...
    if results is not None:
        ...
        st.dataframe(view[cols],hide_index=True,use_container_width=True,height=650,column_config={"카테고리순위":st.column_config.NumberColumn("순위",format="%d"),"계절성점수":st.column_config.ProgressColumn("추천점수",0,100,format="%.1f")})
        if not view.empty:
